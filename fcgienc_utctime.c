...
static char *
strpptime(const char *buf, const char *fmt, struct tm *tm)
{
	char c;
	const char *bp;
	size_t len = 0;
...
literal:
		...
		case 'c':	/* Date and time, using the locale's format. */
			LEGAL_ALT(ALT_E);
			if (!(bp = strpptime(bp, "%x %X", tm)))
				return (0);
			break;

		case 'D':	/* The date as "%m/%d/%y". */
			LEGAL_ALT(0);
			if (!(bp = strpptime(bp, "%m/%d/%y", tm)))
				return (0);
			break;

		case 'R':	/* The time as "%H:%M". */
			LEGAL_ALT(0);
			if (!(bp = strpptime(bp, "%H:%M", tm)))
				return (0);
			break;

		case 'r':	/* The time in 12-hour clock representation. */
			LEGAL_ALT(0);
			if (!(bp = strpptime(bp, "%I:%M:%S %p", tm)))
				return (0);
			break;

		case 'T':	/* The time as "%H:%M:%S". */
			LEGAL_ALT(0);
			if (!(bp = strpptime(bp, "%H:%M:%S", tm)))
				return (0);
			break;
...
static long long diff_tm(struct tm *a, struct tm *b)
{
...
/*
 * param example : <2015-08-11T20:31:17.341Z>
 */
unsigned int time_utc_diff(const char *timestring)
{
	struct tm tmm, localtmm;
	time_t t;
	unsigned int diff;
