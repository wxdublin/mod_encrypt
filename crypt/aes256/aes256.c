...
static int encrypt(EVP_CIPHER_CTX *ctx, unsigned char *plaintext, int plaintext_len, unsigned char *ciphertext, unsigned char *keydata, int keydata_len)
{
	int i, nrounds = 1;
...
int EncryptAesCtr(EVP_CIPHER_CTX *ctx, unsigned char *plaintext, int plainlen, unsigned char *ciphertext, unsigned char *keydata, int keydatalen)
{
	...
	cipher_len = encrypt(ctx, plaintext, plainlen, ciphertext, keydata, keydatalen);
...
int DecryptAesCtr(EVP_CIPHER_CTX *ctx, unsigned char *ciphertext, int cipherlen, unsigned char *decryptedtext, unsigned char *keydata, int keydatalen)
{
	...
	decrypted_len = encrypt(ctx, ciphertext, cipherlen, decryptedtext, keydata, keydatalen);
